...
function simpleMIMOsim(varargin)
  ...
  if isempty(varargin)
    ...
    par.trials = 150; % number of Monte-Carlo trials (transmissions)
    par.SNRdB_list = 12; % list of SNR [dB] values to be simulated
    par.detector = {'SDR RAND','ML'}; % define detector(s) to be simulated
  ...
  res.complexity = zeros(length(par.detector),30);

  % generate random bit stream (antenna x bit x trial)
  bits = randi([0 1],30,par.Q,par.trials);
  NumAntennas=2:5:30;  
  % trials loop
  %tic
  for t=1:par.trials
    ...
    for Mt=NumAntennas
      ...
      idx = bi2de(bits(1:par.MT,:,t),'left-msb')+1;
      s = par.symbols(idx).';
      ...
      for d=1:length(par.detector)
        ...
        res.complexity(d,Mt)=res.complexity(d,Mt)+time_elapsed;
...
%     end      
  ...
  res.complexity= res.complexity/par.trials;
  ...
  for d=1:length(par.detector)
    if d==1   
      semilogy(NumAntennas,res.complexity(d,1:length(NumAntennas)),marker_style{d},'LineWidth',2)
      hold on
    else
      semilogy(NumAntennas,res.complexity(d,1:length(NumAntennas),marker_style{d},'LineWidth',2)
    end
  ...
  hold off
  grid on
  xlabel('Number of Transimitting Antennas M_T','FontSize',12)
